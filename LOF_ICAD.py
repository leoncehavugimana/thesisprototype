...
'''
days_chunks = buildSequences('data/Q', '5min')
length = 12  # Subsequence length
dictOfPvaluesLessThanThreshold = {}  # key is iteration number, value is list of pvalues which are less than the threshold per iteration nber
...
for train_index, test_index in ss.split(days_chunks):
    ...
    dictOfPvaluesLessThanThreshold[iternber] = pvaluesLessThanThresh  # Dict. of p-value values less than threshold per iteration => <key,value> <=> <iternber,list_of_Pvalues_lessThan_threshold>

# ------------------------------------------------------------------------------------
# For each of the 10 folds, Print List_of_Pvalues_lessThan_threshold found per fold
for key in dictOfPvaluesLessThanThreshold:
    print(key, dictOfPvaluesLessThanThreshold[key], '\n')
